...
if (window.jQuery)(function ($) {
	...
	$.fn.MultiFile = function (options) {
		...
		this
			...
			.each(function () {
				...
				MultiFile.E.wrap('<div class="MultiFile-wrap clearfix" id="' + MultiFile.wrapID + '"></div>');
          ...
          MultiFile.wrapper.append('<div class="MultiFile-list clearfix" id="' + MultiFile.wrapID + '_list"></div>');
				...
				MultiFile.addToList = function (slave, slave_count, files) {
					...
					var names = $('<span class="multifile-wrapper"/>');
					$.each(files, function (i, file) {
						var v = String(file.name || '' ).replace(/[&<>'"]/g, function(c) { return '&#'+c.charCodeAt()+';'; }),
								...
								label =	$(
										(
											'<span class="MultiFile-label sub-MultiFile-label" title="' + t + '">'+
												(MultiFile.preview || $(slave).is('.with-preview') ? p : '' )+ '<span class="MultiFile-title">'+ n +'</span>'+
											'</span>'
										)
					...
					//$.each(files, function (i, file) {
						...
						var
							r = $('<div class="MultiFile-label parent-MultiFile"></div>'),
	...
	*/
	$.fn.MultiFile.options = { //$.extend($.fn.MultiFile, { options: {
		...
		// previews
		preview: false,
		previewCss: 'max-height:160px; max-width:200px;',
